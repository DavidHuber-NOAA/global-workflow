...
class GCAFSCycledRocotoXML(RocotoXML):
    ...
    def __init__(self, app_config: AppConfig, rocoto_config: Dict) -> None:
        # Make sure we're using 'gcdas' as the run type
        # First ensure the keys exist before trying to access them
        if 'base' in app_config.configs:
            if app_config.configs['base']['RUN'] == 'gfs':
                app_config.configs['base']['RUN'] = 'gcdas'
            elif 'RUN' not in app_config.configs['base']:
                # If RUN is not defined, set it to 'gcdas'
                app_config.configs['base']['RUN'] = 'gcdas'
            else:
                app_config.configs['base']['RUN'] = 'gcdas'
        else:
            # If 'base' doesn't exist, initialize it with RUN set to 'gcdas'
            app_config.configs['base'] = {'RUN': 'gcdas'}
    ...
    def get_cycledefs(self):
        ...
        strings = []
        strings.append(f'\t<cycledef group="gcdas_half">{sdate_str} {sdate_str} {interval_str}</cycledef>')
        sdate = sdate + interval
        sdate_str = sdate.strftime("%Y%m%d%H%M")
        strings.append(f'\t<cycledef group="gcdas">{sdate_str} {edate_str} {interval_str}</cycledef>')
